...
for k = 1:numel(FireIndx)
  ...
  if cells.burnTime(ii,jj) == cells.flameOut(ii,jj)
    ...
    % horizontal offset
    x0 = @(L, c) log((1-2.*L) ./ (L-1)) ./ c;
    logistic = @(L, c, x) L./(1+exp(-c.*(x-x0(L,c)))); % for no add
    ...
    if (cells.state(ii+1,jj) == 1 && rand < probMatrix(3,2)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj+0];
    end
    % diffuse cells to the west
    if (cells.state(ii-1,jj) == 1 && rand < probMatrix(1,2)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj+0];
    end
    % diffuse cells to the north
    if (cells.state(ii,jj+1) == 1 && rand < probMatrix(2,3)) 
      NewFireIndx = [NewFireIndx ii+0];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the south
    if (cells.state(ii,jj-1) == 1 && rand < probMatrix(2,1)) 
      NewFireIndx = [NewFireIndx ii+0];
      NewFireIndy = [NewFireIndy jj-1];
    end
    % diffuse cells to the northeast
    if (cells.state(ii+1,jj+1) == 1 && rand*sqrt(2) < probMatrix(3,3)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the northwest
    if (cells.state(ii-1,jj+1) == 1 && rand*sqrt(2) < probMatrix(1,3)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the southeast
    if (cells.state(ii+1,jj-1) == 1 && rand*sqrt(2) < probMatrix(3,1)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj-1];
    end
    % diffuse cells to the southwest
    if (cells.state(ii-1,jj-1) == 1 && rand*sqrt(2) < probMatrix(1,1)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj-1];
    end
