...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function vis(cells,time,k,streams,xstart,ystart,flameOutTime,topoPlot,grad) 
% plot
...
if subplots
  ...
  % Optional: add arrows pointing uphill to all burning cells
  fireInd = find(cells.state == 2); % find all burning cells
  [fireIndx, fireIndy] = ind2sub(size(cells.state), fireInd);

  % prevent arrows being drawn on cells in close proximity to eachother
  include = ones(size(fireInd),"logical");
  % for each index, find distances to other indices
  % if too close, set include(index) = 0
  for currentCell = 1:numel(fireInd)
    if include(currentCell)
      distances = pdist2([fireIndx(currentCell), fireIndy(currentCell)], [fireIndx, fireIndy]);
      include(distances < 10) = 0;
      include(currentCell) = 1;
    end
  end
  % keep only indices specified in include
  fireInd = fireInd(include);
  fireIndx = fireIndx(include);
  fireIndy = fireIndy(include);

  % U = x slope, V = y slope. Specifies magnitudes for arrows
  U = grad(fireInd);
  V = grad(fireInd + numel(cells.state));
  quiver(fireIndx, fireIndy, U, V);
