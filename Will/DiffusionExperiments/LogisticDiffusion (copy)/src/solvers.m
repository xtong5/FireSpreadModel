...
o.slopeStrength = prams.slopeStrength;
...
%   disp(bresIgnitions)
  ...
  if cells.burnTime(ii,jj) == cells.flameOut(ii,jj)
    ...
    % get the slope from the current cell to neighboring cells
    slope = cells.topo(ii-1:ii+1, jj-1:jj+1) - cells.topo(ii, jj);
    % obtain the additional probability due to slope
    probMat = logisticProb(o, slope, o.probIgnite, o.slopeStrength);
    probMat = max(probMat, 0); % constrain to positive values only
    % diffuse cells to the east
    if (cells.state(ii+1,jj) == 1 && rand < o.probIgnite + probMat(3, 2)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj+0];
    end
    % diffuse cells to the west
    if (cells.state(ii-1,jj) == 1 && rand < o.probIgnite + probMat(1, 2)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj+0];
    end
    % diffuse cells to the north
    if (cells.state(ii,jj+1) == 1 && rand < o.probIgnite + probMat(2, 3)) 
      NewFireIndx = [NewFireIndx ii+0];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the south
    if (cells.state(ii,jj-1) == 1 && rand < o.probIgnite + probMat(2, 1)) 
      NewFireIndx = [NewFireIndx ii+0];
      NewFireIndy = [NewFireIndy jj-1];
    end
    % diffuse cells to the northeast
    if (cells.state(ii+1,jj+1) == 1 && rand*sqrt(2) < o.probIgnite + probMat(3, 3)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the northwest
    if (cells.state(ii-1,jj+1) == 1 && rand*sqrt(2) < o.probIgnite + probMat(1, 3)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj+1];
    end
    % diffuse cells to the southeast
    if (cells.state(ii+1,jj-1) == 1 && rand*sqrt(2) < o.probIgnite + probMat(3, 1)) 
      NewFireIndx = [NewFireIndx ii+1];
      NewFireIndy = [NewFireIndy jj-1];
    end
    % diffuse cells to the southwest
    if (cells.state(ii-1,jj-1) == 1 && rand*sqrt(2) < o.probIgnite + probMat(1, 1)) 
      NewFireIndx = [NewFireIndx ii-1];
      NewFireIndy = [NewFireIndy jj-1];
    end
...
function y = logisticProb(o, slope, probIgnite, slopeStrength)
  % returns logistic function using the slope as input
  % scales from 0 to 1-probIgnite as slope goes from 0 -> +inf
  y = 2 .* (1-probIgnite) ./ (1 + exp(-slopeStrength .* slope)) ...
    - (1-probIgnite);
end % logisticProb
