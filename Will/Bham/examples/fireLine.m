...
% ensure path for frames exists
if ~exist('frames', 'dir') && saveFrames
  mkdir('frames');
end
...
prams.uwind = 0; 
% prams.uwind = 1.1;
prams.windDir = 50; % wind direction relative to north in degrees
...
elseif prams.sinkStrength ~= 0
  prams.dt = 3*prams.dx;
else
%   prams.dt = min(flameOutTime, [], "all");
  prams.dt = 3*prams.dx;
end
prams.dt = prams.dt * 2;
...
% growth rate of the logistic function that additively increases
% the probability of diffusion when the adjacent cell is uphill.
if exist('strength', 'var')
    prams.slopeStrength = strength;
else
    prams.slopeStrength = 1200;
end

% create linear ridge topography
prams.topo = [linspace(.2, .8, 100) 0.8 linspace(.8, .2, 100)];
prams.topo = repmat(prams.topo, 201, 1);
...
if (usePlot || savePlot) && replayFrames
  cells = geom(prams);
