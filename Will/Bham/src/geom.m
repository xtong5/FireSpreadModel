...
topo     % dimensionless topography 
gradx    % dimensionless x component of topographic slope
grady    % dimensionless y component of topographic slope
...
% calculate gradient from topo
ykernel = [1 0 -1];
xkernel = ykernel';
o.gradx = conv2(o.topo, xkernel, 'same');
o.grady = conv2(o.topo, ykernel, 'same');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function vis(cells,time,k,streams,xstart,ystart,~) 
% plot
...
% plotting of grad for debugging
% if subplots
%   ax2 = subplot(1,2,2);
%   surf(cells.dx*(cells.cx-0.5)*cells.L,...
%       cells.dx*(cells.cy-0.5)*cells.L,cells.grady);
%   view(2); shading flat; axis equal;
%   axis([0 cells.dx*cells.N*cells.L 0 cells.dx*cells.N*cells.L])
%   xticks(linspace(0,cells.N-1,9))
%   yticks(linspace(0,cells.N-1,9))
...
if subplots
  ax2 = subplot(1,2,2);
  surf(cells.dx*(cells.cx-0.5)*cells.L,...
      cells.dx*(cells.cy-0.5)*cells.L,cells.topo);
  view(2); shading flat; axis equal;
  axis([0 cells.dx*cells.N*cells.L 0 cells.dx*cells.N*cells.L])
  xticks(linspace(0,cells.N-1,9))
  yticks(linspace(0,cells.N-1,9))
