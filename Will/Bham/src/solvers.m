...
for k = 1:numel(FireIndx)
  ...
  % cells and going in direction of velocity vector + topographic gradient
  [slopeVecx, slopeVecy] = o.slopeVector(cells, ii, jj);
  [indx,indy] = cells.bham(cells.cx(ii,jj),cells.cy(ii,jj),...
    o.dt/o.dx*(cells.velx(ii,jj)+slopeVecx),...
    o.dt/o.dx*(cells.vely(ii,jj)+slopeVecy));
...
function [x, y] = slopeVector(o, cells, ii, jj)
  x = o.slopeStrength * cells.gradx(ii, jj)^2 * sign(cells.gradx(ii, jj));
  y = o.slopeStrength * cells.grady(ii, jj)^2 * sign(cells.grady(ii, jj));
end % slopeFactor
