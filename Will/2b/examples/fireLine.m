...
prams.uwind = 1.1; % dimensionless speed of wind from the south
...
% prams.slopeStrength = 1;
prams.slopeStrength = 100; % exaggeratted to make more clear
...
topo2 = rot90(fliplr(topo)); % correct for plotting issues
...
gradPlot = rot90(fliplr(sqrt(grady.^2 + gradx.^2)));
...
while time < prams.T && nnz(cells.state == 2)
  ...
  % calculating slope term
  % first combine velocities into array
  vel = reshape([cells.velx, cells.vely], prams.N, prams.N, 2);
  % normalize the wind velocities
  windNorm = vel ./ vecnorm(vel, 2, 3);
  % find slope term, slopeStrength is a constant scalar
  slopeTerm = prams.slopeStrength * dot(grad, windNorm, 3);
  slopeTerm = slopeTerm .* grad;

  % add slope term into velocity
  vel = vel + slopeTerm;

  figure(2); clf; hold on;
  subplot(2,2,1)
  horizFlow = sqrt(cells.velx.^2 + cells.vely.^2);
  horizFlow2 = rot90(fliplr(horizFlow)); % correct for plotting issues
  imagesc(horizFlow2)
  title("Magnitude of Horizontal Flow")
%   title("gradx")
  set(gca, "YDir", "normal");
  axis equal tight;
  colorbar;
  ...
  subplot(2,2,2)
  horizVel = vecnorm(vel, 2, 3);
  horizVel2 = rot90(fliplr(horizVel)); % correct for plotting issues
  imagesc(horizVel2)
  title("Magnitude of Horizontal Velocity")
  set(gca, "YDir", "normal");
  axis equal tight;
  colorbar;
